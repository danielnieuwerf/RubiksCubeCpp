...
bool Cube::operator==(Cube cube) {
	for(int i=0;i<3;++i)
		for (int j = 0; j < 3; ++j) {
			if (top[i][j] != cube.top[i][j])
				return false;
			if (bottom[i][j] != cube.bottom[i][j])
				return false;
			if (north[i][j] != cube.north[i][j])
				return false;
			if (east[i][j] != cube.east[i][j])
				return false;
			if (south[i][j] != cube.south[i][j])
				return false;
			if (west[i][j] != cube.west[i][j])
				return false;
		}
	// If we reach here it is true
	return true;
}
...
void Cube::move(Move m) {
	move(m.initial_face, m.destination_face, m.x_index, m.y_index);
}
...
vector<Move> Cube::getPossibleMoves() {
	// Returns all 18 possible moves
	vector<Move> moves{};
	for (int i = 0; i < 3; ++i) {
		Move m = Move('t', 'n', i, i);
		moves.push_back(m);
